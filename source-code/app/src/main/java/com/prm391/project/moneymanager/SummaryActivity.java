...
public class SummaryActivity extends AppCompatActivity {
   final Context context = this;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sqLiteDatabase.execSQL("create table if not exists savings" +
                "(s_id integer primary key autoincrement,s_target integer,budget_id integer," +
                "foreign key(budget_id) references budget(b_id) on delete cascade);");
        //BUDGET
        Cursor c = sqLiteDatabase.rawQuery("select b_amount,b_cash from budget " +
                "where b_month="+month+" and b_year="+year+";",null);
        if(c.moveToFirst()) {
            int amt = c.getInt(0);
        ...
        //PAID
        c = sqLiteDatabase.rawQuery("select sum(e_amount) from expenses " +
                "where e_mark=1 and budget_id = (select b_id from budget where b_month=" + month + " and b_year=" + year +")", null);
        if(c.moveToFirst()){
           int sum = c.getInt(0);
        ...
        //UNPAID
        c = sqLiteDatabase.rawQuery("select sum(e_amount) from expenses " +
                "where e_mark=0 and budget_id = (select b_id from budget where b_month=" + month + " and b_year=" + year + ")",null);
        if(c.moveToFirst()){
            int sum = c.getInt(0);
        ...
        //LIABILITY
        c = sqLiteDatabase.rawQuery("select e_category_id,e_amount from expenses " +
                "where e_mark=0 and budget_id = (select b_id from budget where b_month=" + month + " and b_year=" + year + ");", null);
        if(c.moveToFirst()){
            do{
                int y=c.getInt(0);
        ...
        //SAVINGS
        c = sqLiteDatabase.rawQuery("select s_target from savings " +
                "where budget_id = (select b_id from budget where b_month = " + month + " and b_year = " + year + ");", null);
        if(c.moveToFirst()){
            int target= c.getInt(0);
            c = sqLiteDatabase.rawQuery("select b_amount,b_cash from budget " +
                    "where b_month =" + month + " and b_year = " + year + ";", null);
            c.moveToFirst();
