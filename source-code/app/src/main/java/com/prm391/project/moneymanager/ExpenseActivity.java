...
public class ExpenseActivity extends AppCompatActivity {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
                ...
                if ((cursor.moveToFirst())) {
                    ...
                    alert.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int whichButton) {
                            ...
                            String amount = input.getEditableText().toString().trim();
                            if (amount.equals("") || Double.parseDouble(amount) <= 0) {
                                Toast.makeText(context, "Invalid Expense", Toast.LENGTH_LONG).show();
                                return;
                            } else {
        ...
        update_expense.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                String array[] = null;
                Cursor cursor = sqLiteDatabase.rawQuery("select c_name from category where c_id in " +
                        "(select e_category_id from expenses where budget_id = " +
                        "(select b_id from budget where b_month=" + month + " and b_year=" + year + ") and e_mark=0);", null);
                if ((cursor.moveToFirst())) {
                    int i = 0;
                    array = new String[cursor.getCount()];
                    do {
                    ...
                    spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                        ...
                        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                            spinner.setSelection(position);
                            category[0] = (String) spinner.getSelectedItem();
                            Cursor c1 = sqLiteDatabase.rawQuery("select e_amount from expenses where e_category_id = " +
                                    "(select c_id from category where c_name='" + category[0] + "') and " +
                                    "budget_id = (select b_id from budget where b_month=" + month + " and b_year=" + year + ");", null);
                            c1.moveToFirst();
                            int amo = c1.getInt(0);
                            input.setHint(String.valueOf(amo));
                        }
                    ...
                    alert.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int whichButton) {
                            ...
                            String amount = input.getEditableText().toString().trim();
                            if (amount.equals("") || Double.parseDouble(amount) <= 0) {
                                Toast.makeText(context, "Invalid Liability", Toast.LENGTH_LONG).show();
                                return;
                            } else {
                                Cursor c1 = sqLiteDatabase.rawQuery("select e_amount from expenses where e_category_id = " +
                                        "(select c_id from category where c_name='" + category[0] + "') and " +
                                ...
                                if ((amo - Integer.parseInt(amount)) < 0) {
                                    Toast.makeText(context, "Invalid Liability", Toast.LENGTH_LONG).show();
                                    return;
                                } else if ((amo - Integer.parseInt(amount)) == 0) {
                                    sqLiteDatabase.execSQL("update expenses set e_mark=1 where  e_category_id = " +
                                            "(select c_id from category where c_name='" + category[0] + "') and " +
                                            "budget_id = (select b_id from budget where b_month=" + month + " and b_year=" + year + ")");
                                    ...
                                    sqLiteDatabase.execSQL("update expenses set e_amount = e_amount - " + amount + "  where e_category_id = " +
                                            "(select c_id from category where c_name='" + category[0] + "') and " +
                                            "budget_id = (select b_id from budget where b_month=" + month + " and b_year=" + year + ")");
                                    sqLiteDatabase.execSQL("update budget set b_cash = b_cash + " + amount + " where b_month=" + month + " and b_year=" + year + ";");
                                    Toast.makeText(context, "Liability Updated", Toast.LENGTH_LONG).show();
        ...
        delete_expense.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Cursor cursor = sqLiteDatabase.rawQuery("select c_name from category  where c_id in " +
                        "(select e_category_id from expenses where budget_id = " +
                        "(select b_id from budget where b_month=" + month + " and b_year=" + year + "));", null);
...
//                            System.out.println(cat[0]);
                            ...
                            c1.moveToFirst();
                            int b_id = c1.getInt(0);
                            Cursor c = sqLiteDatabase.rawQuery("select e_category_id from expenses where budget_id=" + b_id + " and e_category_id = " +
                                    "(select c_id from category where c_name='" + cat[0] + "');", null);
                            c.moveToLast();
                            int x = c.getInt(0);
//                            System.out.println(x);
        ...
        view_expense.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Cursor c = sqLiteDatabase.rawQuery("select e_category_id,e_amount,e_mark from expenses where budget_id = " +
                        "(select b_id from budget where b_month=" + month + " and b_year=" + year + ");", null);
                if (c.moveToFirst() || c.getCount() > 0) {
                    ...
                    builder.setMessage("Expenses for " + (month + 1) + " - " + year );
            ...
            public void onClick(View v) {
                ...
                alert.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int whichButton) {
                        //You will get as string input data in this variable.
                        // here we convert the input to a string and show in a toast.
                        String cat = input.getEditableText().toString().trim();
                        if (cat.equals("")) {
                            Toast.makeText(context, "Invalid Category", Toast.LENGTH_LONG).show();
                            return;
                        } else {
